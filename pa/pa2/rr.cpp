...
vector<pair<int, int>> neighbors(pair<int, int> curr)
{
  ...
  void rainbowRipple(PNG & image, pair<int, int> start, string sgc)
  {
    ...
    vector<vector<int>> D = vec(image.height(), vector<int>(image.width(), 0));

    q.enq(pair<pair<int, int> int>(start, 1));

    while (!q.empty())
    {

      pair<pair<int, int>, int> curr = q.deq();
      D[curr[0][0], curr[0][1]] = -1;

      for (auto &neighbor : neighbors(curr[0]))
      {
        if (good(image, D, curr, neighbor))
        {
          pair<pair<int, int>, int> child(neighbor, curr[1]++);
          q.enq(child);
          // color neighbor
          // TODO
